#include "MapSystem.h"
/*  [TEXTURE_ID - uint8_t] [PROPERTIES uint32_t]
 * uint32_t where each bit represents a component
 * */

MapSystem::MapSystem(std::string mapPath) : map{std::move(mapPath)} {
}

void MapSystem::AddedToWorld(World* world) {
...
void MapSystem::RemovedFromWorld(World* world) {
...
void MapSystem::tick(World* world) {
    ...
    // remove entities that are outside the camera.
    ...
    //Iterate over the `map` in the constructor and instantiate entities
